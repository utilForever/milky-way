...
class Actor:
    def __init__(self, world, props, x, y):
        ...
        self.world = world
        self.sprite = sprite.Sprite(props, x, y)
        self.hitbox = rect.Rect(*(props.get("hitbox", (8, 8, 8, 8))))
        self.velocity_x = 0
        self.velocity_y = 0
    ...
    def check_hit(self, params):
        if (
            self.got_hit is None
            or self.can_get_hit is None
            or self.can_get_hit() is False
        ):
            return
    ...
    def move(self):
        ...
        pos[0], pos[1] = self.push_back(
            pos[0], pos[1], self.velocity_x, self.velocity_y
        )
    ...
    def detect_collision(self, x, y, dy=0):
        ...
        if dy > 0 and pyxel.floor((y + self.hitbox.bottom) % 8) == 1:
            for xi in range(x1, x2 + 1):
    ...
    def set_flip(self):
        if self.set_flip_func:
            ...
            sign = pyxel.sgn(self.velocity_x)
    ...
    def draw(self):
        ...
        if app.g_debug:
            pyxel.rectb(
                pyxel.floor(self.sprite.position[0] + self.hitbox.left - cam.rect.left),
                pyxel.floor(self.sprite.position[1] + self.hitbox.top - cam.rect.top),
                self.hitbox.w,
                self.hitbox.h,
                8,
            )
